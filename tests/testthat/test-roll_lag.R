...
test_that("Grouped lags", {
  ...
  lag2 <- function(x, g, n = 1L) {
    dplyr::pull(
      dplyr::mutate(
        dplyr::tibble(x = x, g = g),
        lag = dplyr::lag(x, n = n), .by = g
      ), lag
    )
  }
...
test_that("More complicated diff tests", {
  x <- rnorm(10^3)
  y <- as.integer(x)
  z <- x > 0
  o1 <- seq_along(x)
