...
While most time-based packages are designed to work with clean and pre-aggregate data,   
timeplyr contains a set of tidy tools to complete, expand and summarise both raw and aggregate date/datetime data.
...
These include: `time_countv`, `time_expandv`, `time_summarisev` and `time_completev`.
...
Currently it vectorised over from, to and by
...
These functions are wrappers around the `collapse` equivalents, but the output 
