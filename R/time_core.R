...
#' @rdname time_core
#' @export
time_grid <- function(x, timespan = NULL, from = NULL, to = NULL){
  check_is_time_or_num(x)
  check_length_lte(from, 1)
  check_length_lte(to, 1)
  time_by <- get_time_granularity(x, timespan)
  if (is.null(from)){
...
#' @rdname time_core
#' @export
time_completev <- function(x, time_by = NULL,
                           sort = TRUE,
                           roll_month = getOption("timeplyr.roll_month", "preday"),
                           roll_dst = getOption("timeplyr.roll_dst", "NA")){
  time_full <- time_grid(x, time_by = time_by,
...
#' @rdname time_core
#' @export
time_summarisev <- function(x, timespan = NULL, from = NULL, to = NULL,
                            sort = FALSE, unique = FALSE){
  check_is_time_or_num(x)
  check_length_lte(from, 1)
  check_length_lte(to, 1)
  timespan <- get_time_granularity(x, timespan)
  ...
  # Time sequence
  time_breaks <- time_grid(x, timespan, from = from, to = to)
  x <- time_cast(x, time_breaks)
  to <- time_cast(to, x)

  out <- cut_time(x, breaks = time_breaks, codes = FALSE, include_oob = T)
  if (unique){
  ...
  new_time_interval(out, timespan)
}
#' @rdname time_core
#' @export
time_countv <- function(x, timespan = NULL, from = NULL, to = NULL,
