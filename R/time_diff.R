...
#     if (time_type == "period"){
#       # unit <- period_unit(units)(abs(num))
#       # n_unique_units <- n_unique(unit)
#       # if (n_unique_units <= 1){
#       #   unit <- unit[n_unique_units]
#       #   num <- num[n_unique_units]
#       # }
#       # Use distinct start/end pairs (intervals)
#       # Instead of all of them because it's usually more efficient
#       interval_tbl <- list_to_data_frame(recycle_args(x = x, y = y))
#       # interval_groups <- df_to_GRP(interval_tbl, .cols = c("x", "y"), order = FALSE)
#       interval_groups <- collapse::group(interval_tbl, starts = TRUE, group.sizes = TRUE)
#       starts <- attr(interval_groups, "starts")
#       sizes <- attr(interval_groups, "group.sizes")
#       n_groups <- attr(interval_groups, "N.groups")
...
#         interval_tbl <- df_row_slice(interval_tbl, starts)
#       }
#       # distinct_pairs <- isTRUE((df_nrow(interval_tbl) %/% GRP_n_groups(interval_groups)) >= 2L)
#       # if ( distinct_pairs ){
#       #   interval_tbl <- df_row_slice(interval_tbl, GRP_starts(interval_groups))
#       # }
#       x <- interval_tbl$x
#       y <- interval_tbl$y
#
...
#       if (distinct_pairs){
#         out <- out[interval_groups]
#         # out <- out[group_id(interval_groups)]
#       }
#     } else {
