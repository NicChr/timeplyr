...
## An alternative that accepts a range but this doesn't
## really work because the gcd(diff(x)) is needed as well
## which can't be calculated from the range alone
## And so the output is going to be different.

# ggplot_time_breaks <- function(rng, n = 5, time_by = NULL,
#                                time_floor = FALSE,
#                                week_start = getOption("lubridate.week.start", 1),
#                                time_type = getOption("timeplyr.time_type", "auto"),
#                                roll_month = getOption("timeplyr.roll_month", "preday"),
#                                roll_dst = getOption("timeplyr.roll_dst", "boundary")){
#   check_is_time_or_num(rng)
#   check_is_num(n)
#   stopifnot(n >= 1)
#   check_length(n, 1L)
#   time_type <- rlang::arg_match0(time_type, c("auto", "duration", "period"))
#   rng2 <- as.double(rng)
#   if(rng2[1] <= 0){
#     rng2 <- rng2 + c(-rng2[1], rng2[1])
#   } else{
#     rng2 <- rng2 + c((rng2[2]- rng2[1]) * 0.04545455,
#                     -(rng2[2] - rng2[1]) * 0.04545455)
#   }
#   rng <- time_cast(rng2, rng)
#   from <- rng[1]
#   to <- rng[2]
#   del <- diff(rng2)
#   if (is.null(time_by)){
