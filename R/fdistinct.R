...
#   out <- fselect(out, .cols = out_vars)
#   g <- df_to_GRP(out, .cols = dup_vars, order = order)
#   # This if faster but only works when the above line has order = sort
#   # The reason for not doing this is when data is large and messy,
#   # Sorting it can be much faster
#   # group_data <- GRP_groups(g)
#   # if (is.null(group_data) || .keep_all){
#   #   group_start_rows <- GRP_starts(g)
#   #   out <- df_row_slice(out, group_start_rows, reconstruct = FALSE)
#   # } else {
#   #   out <- group_data
#   # }
#   order_sort_alike <- !order || (order && sort)
...
#     group_start_rows <- GRP_starts(g)
#   } else {
#     # group_start_rows <- collapse::whichv(GRP_duplicated(g), FALSE)
#     group_start_rows <- !GRP_duplicated(g)
#     # Which row numbers (by-group) are equal to 1?
#     # if (length(dup_vars) == 0L){
#     #   g <- NULL
#     # }
#     # group_start_rows <- collapse::whichv(frowid(out, g = g), 1L)
#   }
#   if (order_sort_alike && GRP_has_groups && !.keep_all){
#     out <- GRP_groups(g)
#   } else {
#     out <- df_row_slice(out, group_start_rows, reconstruct = FALSE)
#   }
