...
//     return NA_INTEGER;
//   }
//   int64_t res = (int64_t) y - x;
//   if (res > r_integer_max_ || res < r_integer_min_){
//     return NA_INTEGER;
//   } else {
//     return y - x;
//   }
...
// cpp_diff accepts vectors with > 2^31 - 1 elements but
// if order is supplied then both must be <= 2^31 - 1
...
//
//       // To-do: get the else section to work with negative lags
//       if (recycle_lag || lag1 < 0){
...
//       } else {
//         if (std::abs(lag1) >= 1){
//           SEXP lag_temp = Rf_protect(Rf_allocVector(INTSXP, std::abs(lag1)));
//           ++n_prot;
...
//       } else {
//         if (std::abs(lag1) >= 1){
//           SEXP lag_temp = Rf_protect(Rf_allocVector(REALSXP, std::abs(lag1)));
//           ++n_prot;
